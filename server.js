...
async function getAccessToken() {
  ...
  if (!res.ok || !data.access_token) throw new Error("Failed to get QPay token");
...
async function createInvoice({ amount, email, testType }) {
  ...
  try {
    ...
    console.error("❌ QPay JSON parse error:", e);
    throw new Error("Invalid QPay response");
  }

  if (!res.ok || !data.invoice_id) throw new Error("Failed to create invoice");
  return data;
}
...
// ---------------------------
function getInterpretation(testType, score) {
...
if (testType === "sectionsTest1") {
  ...
  // Add other test types similarly (sectionsTest3–7)
...
app.post("/start-payment", async (req, res) => {
  try {
    const { email, testType, amount } = req.body;
    const invoice = await createInvoice({ email, testType, amount });
    res.json({ success: true, invoice_id: invoice.invoice_id, urls: invoice.urls, qr_image: invoice.qr_image });
  } catch (err) {
...
// ---------------------------
// Check payment and send results
// ---------------------------
